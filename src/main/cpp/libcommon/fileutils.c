...
char * parse_format(const char * format, int tracknum, const char * year, const char * artist, const char * album, const char * title)
{
    ...
    if (ret == NULL)
        LOGE("malloc(sizeof(char) * (len+1)) failed. Out of memory.");
...
int recursive_mkdir(char* path_and_name,char * base_dir, mode_t mode)
{
    ...
    LOGD("NOTICE in fileutils:recursive_mkdir before call mkdir..path_and_name: %s; base_dir:%s; path_and_name_length:%d", path_and_name, base_dir, path_and_name_length);
    ...
    for (count = 0; count < path_and_name_length; count++)
    {
        if (pos[count] == '/' || pos[count] == '\\')
        {
            ...
            LOGD("%s recursive_mkdir after call mkdir..path_and_name: %s; return=%d", __func__, path_and_name, rc);
    ...
    LOGD("NOTICE in fileutils:recursive_mkdir after call mkdir..path_and_name: %s; return=%d", path_and_name, rc);
