...
void
CFrameReader::readDSDFrame(CStrData &SD, long MaxFrameLen, int NrOfChannels, uint8_t *DSDFrame) {
...
void
CFrameReader::readTableSegmentData(CStrData &SD, int NrOfChannels, int FrameLen, int MaxNrOfSegs,
                                   int MinSegLen, CSegment &S, int &SameSegAllCh) {
    int ChNr = 0;
    int DefinedBits = 0;
...
void CFrameReader::readSegmentData(CStrData &SD, CFrameHeader &FH) {
    ...
    readTableSegmentData(SD, FH.NrOfChannels, FH.MaxFrameLen, MAXNROF_FSEGS, MIN_FSEG_LEN, FH.FSeg,
                         FH.FSameSegAllCh);

    if (FH.PSameSegAsF == 1) {
        copySegmentData(FH);
    } else {
        readTableSegmentData(SD, FH.NrOfChannels, FH.MaxFrameLen, MAXNROF_PSEGS, MIN_PSEG_LEN,
...
void
CFrameReader::readTableMappingData(CStrData &SD, int NrOfChannels, int MaxNrOfTables, CSegment &S,
                                   int &NrOfTables, int &SameMapAllCh) {
    int CountTables = 1;
    int NrOfBits = 1;
...
void CFrameReader::readMappingData(CStrData &SD, CFrameHeader &FH) {
    ...
    readTableMappingData(SD, FH.NrOfChannels, FH.MaxNrOfFilters, FH.FSeg, FH.NrOfFilters,
                         FH.FSameMapAllCh);

    if (FH.PSameMapAsF == 1) {
        copyMappingData(FH);
    } else {
        readTableMappingData(SD, FH.NrOfChannels, FH.MaxNrOfPtables, FH.PSeg, FH.NrOfPtables,
                             FH.PSameMapAllCh);
    }
...
void CFrameReader::readFilterCoefSets(CStrData &SD, int NrOfChannels, CFrameHeader &FH,
                                      CCodedTableF &CF) {
    ...
    for (int FilterNr = 0; FilterNr < FH.NrOfFilters; FilterNr++) {
        ...
        if (!CF.Coded[FilterNr]) {
            ...
            for (int CoefNr = CF.CPredOrder[bestmethod];
...
void CFrameReader::readProbabilityTables(CStrData &SD, CFrameHeader &FH, CCodedTableP &CP,
    ...
    for (int PtableNr = 0; PtableNr < FH.NrOfPtables; PtableNr++) {
        ...
        if (FH.PtableLen[PtableNr] > 1) {
            ...
            if (!CP.Coded[PtableNr]) {
                ...
                for (int EntryNr = CP.CPredOrder[bestmethod];
