...
// PREDICTION FILTER
#define SIZECODEDPREDORDER 7 // Number of bits for representing the prediction order (for each channel) in each frame SIZECODEDPREDORDER = 7 (128)
#define MAXPREDORDER (1<<(SIZECODEDPREDORDER)) // Maximum prediction filter order
#define SIZEPREDCOEF 9 // Number of bits for representing each filter coefficient in each frame
#define PFCOEFSCALER ((1 << ((SIZEPREDCOEF) - 1)) - 1) // Scaler for coefficient normalization

// P-TABLES
#define SIZECODEDPTABLELEN 6 // Number bits for p-table length
#define MAXPTABLELEN (1<<(SIZECODEDPTABLELEN)) // Maximum length of p-tables

// P-table indices
#define MAXPTIND ((MAXPTABLELEN)-1) // Maximum level for p-table indices
#define ACQSTEP ((SIZEPREDCOEF)-(SIZECODEDPTABLELEN)) // Quantization step for histogram

// P-table values
#define SIZEPROBS 8 // Number of bits for coding probabilities
#define PROBLEVELS (1<<(SIZEPROBS)) // Number of probability levels / Maximum level: ((PROBLEVELS)-1)

// ARITHMETIC CODE
#define SIZEADATALENGTH 18 // Number of bits for representing the length of the arithmetic coded sequence
#define MAXADATALEN (1<<(SIZEADATALENGTH)) // Maximum length of Arithmetic encoded data
...
// Maximum length of encoded frame in bits / number of output words in encoded stream = MAXENCFRAMELEN / ENCWORDLEN
