...
int CDSTDecoder::decode(uint8_t *DSTFrame, int frameSize, uint8_t *DSDFrame) {
    ...
    if (FrameHdr.DSTCoded == 1) {
        ...
        if (ACError != 1) {
            LOGD("ERROR: Arithmetic decoding error!");
            rv = -1;
        }
    }
...
int CDSTDecoder::unpack(uint8_t *DSTFrame, uint8_t *DSDFrame) {
    ...
    if (FrameHdr.DSTCoded == 0) {
        ...
        if (Dummy != 0) {
            LOGD("ERROR: Illegal stuffing pattern in frame %d!\n", FrameHdr.FrameNr);
        ...
        if (ADataLen > 0 && GET_BIT(AData, 0) != 0) {
            LOGD("ERROR: Illegal arithmetic code in frame %d!", FrameHdr.FrameNr);
            return -1;
        }
    }
...
// Take the 7 LSBs of a number consisting of SIZE_PREDCOEF bits (2's complement), reverse the bit order and add 1 to it.
...
void
CDSTDecoder::fillTable4Bit(CSegment &S, uint8_t Table4Bit[MAX_CHANNELS][MAX_DSDBITS_INFRAME / 2]) {
    int SegNr;
    int Start;
    int End;
    int8_t Val;
