...
namespace chromaprint {
    ...
    inline bool FFmpegAudioReader::Open(const std::string &file_name) {
        ...
        if (m_codec_ctx->sample_fmt != AV_SAMPLE_FMT_S16 ||
            m_codec_ctx->ch_layout.nb_channels != m_output_channels ||
            m_codec_ctx->sample_rate != m_output_sample_rate) {
            ...
            if (m_frame->nb_samples > 0) {
                if (m_converter) {
                    if (m_frame->nb_samples > m_convert_buffer_nb_samples) {
                        ...
                        ret = av_samples_alloc(m_convert_buffer, &linsize,
                                               m_codec_ctx->ch_layout.nb_channels,
                                               m_convert_buffer_nb_samples, AV_SAMPLE_FMT_S16, 1);
                        if (ret < 0) {
                ...
                if (m_converter) {
                    if (IsFinished()) {
                        auto nb_samples = m_converter->Flush(m_convert_buffer,
                                                             m_convert_buffer_nb_samples);
                        if (nb_samples < 0) {
                            SetError("Couldn't convert audio", ret);
                            return false;
