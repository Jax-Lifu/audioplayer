...
static int dst_decoder_thread_create(dst_decoder_thread_t decoder, int spu_id, sys_event_queue_t recv_queue)
{
    ...
    if (ret != 0) 
    {
        LOGE("sysEventQueueCreate failed: %#.8x, %d", ret, spu_id);
        return ret;
    }
    ...
    if (ret != 0) 
    {
        LOGE("sysEventPortCreate failed: %#.8x, %d", ret, spu_id);
        return ret;
    }
    ...
    if (ret != 0) 
    {
        LOGE("sysEventPortConnectLocal failed: %#.8x, %d", ret, spu_id);
        return ret;
    }
    ...
    if (ret != 0) 
    {
        LOGE("sysSpuThreadGroupCreate failed: %#.8x, %d", ret, spu_id);
        return ret;
    }
    ...
    if (ret != 0) 
    {
        LOGE("sysSpuImageOpen failed: %#.8x, %d", ret, spu_id);
        return ret;
    }
    ...
    if (ret != 0) 
    {
        LOGE("sysSpuThreadInitialize failed: %#.8x, %d", ret, spu_id);
        return ret;
    }
    ...
    if (ret != 0) 
    {
        LOGE("sysSpuThreadConnectEvent failed: %#.8x, %d", ret, spu_id);
        return ret;
    }
    ...
    if (ret != 0) 
    {
        LOGE("sysSpuThreadBindQueue failed: %#.8x, %d", ret, spu_id);
        return ret;
    }
    ...
    if (ret != 0) 
    {
        LOGE("sysSpuThreadGroupStart failed: %#.8x, %d", ret, spu_id);
        return ret;
    }
...
static int dst_decoder_thread_destroy(dst_decoder_thread_t decoder)
{
    ...
    if (ret != 0) 
    {
        LOGE("sysSpuThreadGroupJoin failed: %#.8x, %d", ret, spu_id);
    }
    ...
    if (ret != 0) 
    {
        LOGE("sysSpuThreadDisconnectEvent() failed: %#.8x, %d", ret, spu_id);
    }

    ret = sysSpuThreadUnbindQueue(decoder->spu_thread, DST_QUEUE_NUMBER);
    if (ret != 0) 
    {
        LOGE("sysSpuThreadUnbindQueue(%u, %u) failed: %#.8x, %d",
    ...
    if (ret != 0) 
    {
        LOGE("sysSpuThreadGroupDestroy() failed: %#.8x, %d", ret, spu_id);
    }

    ret = sysEventPortDisconnect(decoder->event_port);
    if (ret != 0) 
    {
        LOGE("sysEventPortDisconnect() failed: %#.8x, %d", ret, spu_id);
    }

    ret = sysEventPortDestroy(decoder->event_port);
    if (ret != 0) 
    {
        LOGE("sysEventPortDestroy() failed: %#.8x, %d", ret, spu_id);
    }

    ret = sysEventQueueDestroy(decoder->send_queue, 0);
    if (ret != 0) 
    {
        LOGE("sysEventQueueDestroy failed: %#.8x, %d", ret, spu_id);
    }

    ret = sysSpuImageClose(&decoder->spu_img);
    if (ret != 0) 
    {
        LOGE("sysSpuImageClose failed: %.8x, %d", ret, spu_id);
    }
...
static int dst_decoder_wait(dst_decoder_t *dst_decoder, int timeout)
{
    ...
    while (event_count > 0)
    {
        ...
        if (ret != 0) 
        {
            LOGE("sysEventQueueReceive failed: %#.8x", ret);
            errors++;
        } 
        else 
        {
...
dst_decoder_t* dst_decoder_create(int channel_count, frame_decoded_callback_t frame_decoded_callback, frame_error_callback_t frame_error_callback, void *userdata)
{
    ...
    if (ret != 0) 
    {
        LOGE("sysEventQueueCreate failed: %#.8x", ret);
        return 0;
    }
    ...
    for (i = 0; i < NUM_DST_DECODERS; i++)
    {
        ...
        if (ret != 0)
        {
            LOGE("dst_decoder_thread_create failed: %#.8x", ret);
            return 0;
        }
    ...
    if (ret != 0)
    {
        LOGE("dst_decoder_wait failed: %#.8x", ret);
        return 0;
    }
...
int dst_decoder_destroy(dst_decoder_t *dst_decoder)
{
    ...
    for (i = 0; i < NUM_DST_DECODERS; i++)
    {
        ...
        if (ret != 0) 
        {
            LOGE("sysEventPortSend failed: %#.8x", ret);
        }

        ret = dst_decoder_thread_destroy(dst_decoder->decoder[i]);
        if (ret != 0)
        {
            LOGE("dst_decoder_thread_destroy failed: %#.8x", ret);
        }
    ...
    if (ret != 0) 
    {
        LOGE("sysEventQueueDestroy failed: %#.8x", ret);
    }
...
int dst_decoder_decode(dst_decoder_t *dst_decoder, uint8_t *dst_data, size_t dst_size)
{
    ...
    if (dst_decoder->event_count > NUM_DST_DECODERS)
    {
        LOGE("dst_decoder->event_count > NUM_DST_DECODERS");
