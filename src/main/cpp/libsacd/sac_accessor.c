...
int create_sac_accessor(void)
{
    ...
    if (sa == NULL)
    {
        LOGE("sac_accessor_t malloc failed\n");
        return -1;
    }
    sa->id = -1;
    ...
    if (ret != 0)
    {
        LOGE("cannot load file: " SAC_MODULE_LOCATION "0x%x\n", ret);
        return 0;
    }
    ...
    if (ret != 0)
    {
        LOGE("sys_isoself_spu_create : 0x%x\n", ret);
        return 0;
    }
#else
    ...
    if (ret)
    {
        LOGE("sysSpuRawCreate failed %d\n", ret);
        return ret;
    }
    LOGD("succeeded. raw_spu number is %d\n", sa->id);
    ...
    if (ret)
    {
        LOGE("sysSpuRawLoad failed [" SAC_MODULE_LOCATION "]%d\n", ret);
        return ret;
    }
    LOGD("succeeded. entry %x\n", entry);
#endif
    ...
    if (ret != 0)
    {
        LOGE("sys_isoself_spu_set_int_mask : 0x%x\n", ret);
        return 0;
    }
#else
    ...
    if (ret != 0)
    {
        LOGE("sys_raw_spu_set_int_mask : 0x%x\n", ret);
        return ret;
    }
#endif
    ...
    {
        LOGE("create mmio_mutex failed.\n");
        return -1;
    }

    if (sysCondCreate(&sa->mmio_cond, sa->mmio_mutex, &cond_attr) != 0)
    {
        LOGE("create mmio_cond failed.\n");
        return -1;
    }
    ...
    if ((ret = sysThreadCreate(&sa->handler, handle_interrupt, 0, PRIMARY_PPU_THREAD_PRIO,
        ...
        != 0)
    {
        LOGE("ppu_thread_create returned %d\n", ret);
        return ret;
    }
    ...
    if (ret != 0)
    {
        LOGE("sys_isoself_spu_create_interrupt_tag : 0x%x\n", ret);
        return 0;
    }
#else
    ...
    if (ret != 0)
    {
        LOGE("sys_raw_spu_create_interrupt_tag : 0x%x\n", ret);
        return ret;
    }
#endif

    // Establishing the interrupt tag on the interrupt PPU thread.
    LOG("Establishing the interrupt tag on the interrupt PPU thread.\n");
    if ((ret = sysInterruptThreadEstablish(&sa->ih, sa->intrtag, sa->handler,
                                           sa->id)) != 0)
    {
        LOGE("sys_interrupt_thread_establish returned %d\n", ret);
        return ret;
    }
    ...
    if (ret != 0)
    {
        LOGE("sys_isoself_spu_start : 0x%x\n", ret);
        return 0;
    }
#else
...
int destroy_sac_accessor(void)
{
    ...
    {
        LOGE("sysInterruptThreadDisestablish returned %d\n", ret);
    }

    if ((ret = sysInterruptTagDestroy(sa->intrtag)) != 0)
    {
        LOGE("sys_interrupt_tag_destroy returned %d\n", ret);
    }
    ...
    if (sa->id != -1)
    {
        ...
        {
            LOGE("sys_isoself_spu_destroy returned %d\n", ret);
        }
#else
        if ((ret = sysSpuRawDestroy(sa->id)) != 0)
        {
            LOGE("sys_raw_spu_destroy returned %d\n", ret);
        }
#endif
    }
    ...
    {
        LOGE("destroy mmio_cond failed.\n");
    }

    if ((ret = sysMutexDestroy(sa->mmio_mutex)) != 0)
    {
        LOGE("destroy mmio_mutex failed.\n");
    }
...
void handle_interrupt(void *arg)
{
    ...
    if (ret != 0)
    {
        LOGE("sys_isoself_spu_get_int_stat failed %d\n", ret);
    ...
    if (ret != 0)
    {
        LOGE("sys_raw_spu_get_int_stat failed %d\n", ret);
        sysInterruptThreadEOI();
    }
#endif
    ...
    if ((stat & INTR_PPU_MB_MASK) == INTR_PPU_MB_MASK)
    {
        ...
        if (ret != 0)
        {
            LOGE("sys_isoself_spu_read_puint_mb failed %d\n", ret);
        ...
        if (ret != 0)
        {
            LOGE("sys_isoself_spu_set_int_stat failed %d\n", ret);
            sysMutexUnlock(sa->mmio_mutex);
            sysInterruptThreadEOI();
        }
#else
        ...
        if (ret != 0)
        {
            LOGE("sys_raw_spu_read_puint_mb failed %d\n", ret);
            sysMutexUnlock(sa->mmio_mutex);
            sysInterruptThreadEOI();
        }
        ...
        if (ret != 0)
        {
            LOGE("sys_raw_spu_set_int_stat failed %d\n", ret);
            sysMutexUnlock(sa->mmio_mutex);
            sysInterruptThreadEOI();
        }
#endif
                  ...
                  , uint32_t timeout)
{
    ...
    if (ret != 0)
    {
        LOGE("exchange_data: [sysCondWait timeout]\n");
        sysMutexUnlock(sa->mmio_mutex);
        return ret;
    }
...
int sac_exec_key_exchange(int fd)
{
    ...
    LOGD("sac_exec_generate_key 0 %x %x\n", buffer_size, ret);
    if (ret != 0)
    {
        return ret;
    }
    ...
    LOGD("sac_exec_validate_key_1[%x]\n", ret);
    if (ret != 0)
    {
        return ret;
    }
    ...
    LOGD("sac_exec_generate_key_2[%x] %x\n", buffer_size, ret);
    if (ret != 0)
    {
        return ret;
    }
    ...
    LOGD("sac_exec_validate_key_2[%x]\n", ret);
    if (ret != 0)
    {
        return ret;
    }
    ...
    LOGD("sac_exec_validate_key_3[%x]\n", ret);
    if (ret != 0)
    {
        return ret;
    }
    ...
    LOGD("ioctl_report_finish [0xff] %x\n", ret);
...
int file_alloc_load(const char *file_path, uint8_t **buf, unsigned int *size)
{
    ...
    if (ret != 0)
    {
        LOGE("file %s get stat error : 0x%x\n", file_path, ret);
        sysFsClose(fd);
        return -1;
    }
