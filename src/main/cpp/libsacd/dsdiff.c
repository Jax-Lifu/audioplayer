...
static int dsdiff_create_edit_master(scarletbook_output_format_t *ft)
{
	...
	{ 		        
		LOGD("dsdiff_create_edit_master(): error writing in file %s", ft->filename);
		return -1;               				
	}
    return ret;
}
...
static int dsdiff_create(scarletbook_output_format_t *ft)
{
    ...
    dsdiff_handle_t  *handle = (dsdiff_handle_t *) ft->priv;
    size_t nrw=fwrite(handle->header, 1, handle->header_size, ft->fd);
    LOGD("dsdiff_create : writing in file %s", ft->filename);
	if(nrw !=  handle->header_size)
	{ 		        
		LOGD("dsdiff_create(0: error writing in file %s", ft->filename);
		return -1;               				
	}	
    return ret;
}
...
{
    dsdiff_handle_t *handle = (dsdiff_handle_t *) ft->priv;
    LOGD("dsdiff_close : writing in file %s", ft->filename);
    ...
    if (handle->audio_data_size % 2)
    {
		...
		if(nrw != 1)
		{ 		        
			LOGD("dsdiff_close(0: error writing in file %s", ft->filename);
			return -1;               				
		}
...
static int dsdiff_write_frame(scarletbook_output_format_t *ft, const uint8_t *buf, size_t len)
{
    ...
    if (ft->dsd_encoded_export)
    {
        ...
        //LOGD("dsdiff_write_frame(): writing in file %s len %d %d", ft->filename, len, nrw);
		if(nrw != len)
		{ 		        
			LOGD("dsdiff_write_frame(): error writing in file %s", ft->filename);
			return -1;               				
		}
    ...
    else
    {
        ...
        dst_frame_data_chunk.chunk_data_size = hton64(len);
        {
			...
			{ 		        
				LOGD("dsdiff_write_frame(0: error writing in file %s", ft->filename);
				return -1;               				
			}
