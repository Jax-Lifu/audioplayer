...
sacd_reader_t *sacd_open_callbacks(sacd_io_callbacks_t *callbacks) {
    ...
    // Call the input layer to setup callbacks
    dev = sacd_input_open_callbacks(callbacks);
    if (!dev) {
        fprintf(stderr, "libsacdread: Can't open via callbacks\n");
        return NULL;
    }
    ...
    // Treat stream as an image file (random access allowed via seek)
